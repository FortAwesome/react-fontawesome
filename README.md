...
Notice that the `faCoffee` icon is imported from `@fortawesome/fontawesome-free-solid` as an object and then
...
`faCheckSquare` and `faCoffee`. We also add all of the brands in `@fortawesome/fontawesome-free-brands`.
...
* `fab`: which represents _all_ of the brand icons in `@fortawesome/fontawesome-free-brands`. So any of the brand icons in that package may be referenced by icon name as a string anywhere else in our app. For example: `"apple"`, `"microsoft"`, or `"google"`.
...
* We used the `"apple"`, `"microsoft"`, and `"google"` brand icons, which were never explicitly _individually_ imported, but they're available to us by name as strings because `fab` was added to our library in `App.js`, and `fab` includes all of those icons.
...
So we need to specify a different prefix for them—not the default `fas`, but `fab`, for Font Awesome _Brand_.
...
underlying API library, `@fortawesome/fontawesome`.
...
package. They are re-exported from both `@fortawesome/fontawesome`
